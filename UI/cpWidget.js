...
const defaultCpsList = (cps, NbCheckpoints, login) => {
  ...
  let cpList = cps.reduce((acc, res, index) => {
    if (wayPoints[index]) {
    ...
    const newLabel = `
      <label pos="155 ${height}" textsize="1.2" textcolor="${colors[index] || 'FFFFFF'}" text="${index} - ${raceTime(res.time)}" z-index="2" textfont="RajdhaniMono" halign="right" valign="top"/>
    `;
